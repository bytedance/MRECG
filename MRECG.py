...
def node2modules(name2modules, nodes):
    r"""
    Create a dictionary mapping of GraphModule node to the corresponding nn.Module.
    """
    modules = dict()
    for node in nodes:
...
def qnode2fpnode(quant_modules, fp32_modules):
    r"""
    For the GraphModule, create a dictionary mapping of quantized nodes to the corresponding full precision nodes.
    """
    quant_named_nodes = {node.name: node for node in quant_modules}
...
class LinearTempDecay:
    r'''linear decrement quantization parameter b.
    '''
...
class CosineTempDecay:
    r'''cosine decrement quantization parameter b.
    '''
    def __init__(self, t_max=10000, warm_up=0.2, start_b=20, end_b=2):
        self.t_max = t_max
        self.start_decay = warm_up * t_max
...
def subgraph_reconstruction(subgraph, cached_inps, cached_oups, config):
    r"""
    PTQ reconstruction based on subgraph, inputs and full precision output.
    """
    global USE_LINK
    global USE_DDP
...
def extract_layer(node, fp32_modules):
    r"""
    Given a node in a GraphModule, output the layer node list that contain this node.

    Args:
        node (torch.fx.Node): The given node is used to extract the module list
        fp32_modules (torch.nn.Module): Candidate model to be extracted
    ...
    while True:
        ...
        if "cat_" not in cur_node.name or "quantizer" in cur_node.name:
...
def extract_block(input_nodes, fp32_modules, depth=0, scheme=()):
    r"""
    Given a node in a GraphModule, output the module node list that contain this node based on the scheme.

    Args:
        input_nodes (List): The given nodes are used to extract the module list
        fp32_modules (torch.nn.Module): Candidate model to be extracted
        depth (int): Current module extraction depth
        scheme (int or tuple): The scheme of extracting modules, i.e., whether to optimize jointly with adjacent modules
    ...
    if type(scheme)==tuple and depth == 2:
        if len(scheme) == 2:
            return layer_node_list + exp_nodes + extract_block(
                [exp_nodes[-1]], fp32_modules, 0, scheme=1)
        elif len(scheme) == 3:
            return layer_node_list + exp_nodes + extract_block(
                [exp_nodes[-1]], fp32_modules, 0, scheme=(1,1))
    elif type(scheme)==tuple and (cur_node.name in ["add", "add_1", "add_2", "add_3", "add_4", "add_5", "add_6", "add_7", "add_8", "add_9", "add_12", "add_14"]):
        if len(scheme) == 2:
            return layer_node_list + exp_nodes + extract_block(
                [exp_nodes[-1]], fp32_modules, 0, scheme=1)
        elif len(scheme) == 3:
...
def ptq_reconstruction(model: GraphModule, cali_data: list, config: dict, graph_module_list: list = None):
    ...
    config.scheme = eval(config.scheme)
    fp32_model = model
    fp32_model.eval()
    ...
    for node in nodes:
        ...
        if node.op == "call_module" and isinstance(quant_modules[node], _ADAROUND_SUPPORT_TYPE):
            ...
            if config.pattern == 'layer':
                ...
                layer_node_list = extract_block(node.all_input_nodes, quant_modules, scheme=config.scheme[count])
                count += 1
            else:
            ...
            subgraph.class_name = layer_node_list[-1].name
